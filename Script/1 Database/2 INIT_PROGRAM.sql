...
-- drop trigger QL_TRUONGHOC_X.TR_UPDATE_SOTINCHITICHLUY;
CREATE OR REPLACE TRIGGER QL_TRUONGHOC_X.TR_INSERT_SOTINCHITICHLUY
...
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        RAISE_APPLICATION_ERROR(-20001, 'Khong tom thay du lieu khmo.' || SQLERRM);
    WHEN OTHERS THEN
...
AFTER UPDATE ON QL_TRUONGHOC_X.DANGKY
FOR EACH ROW
DECLARE 
    PRAGMA AUTONOMOUS_TRANSACTION;
    sotc number(3,0);
BEGIN
    SELECT SOTC INTO sotc
        ...
        WHERE hp.MAHP = :new.MAHP and ROWNUM <= 1;
    UPDATE QL_TRUONGHOC_X.SINHVIEN sv
    SET sv.DTBTL = (sv.DTBTL * sv.SOTCTL + :new.DIEMTK * sotc) / ( sv.DTBTL * sv.SOTCTL +  sotc )
    WHERE sv.MASV = :new.MASV;
    COMMIT;  
EXCEPTION
    WHEN NO_DATA_FOUND THEN
...
-- drop trigger QL_TRUONGHOC_X.TR_DELETE_SOTINCHITICHLUY
--CREATE OR REPLACE TRIGGER QL_TRUONGHOC_X.TR_DELETE_SOTINCHITICHLUY
--AFTER DELETE ON QL_TRUONGHOC_X.DANGKY
--FOR EACH ROW
--DECLARE 
--    PRAGMA AUTONOMOUS_TRANSACTION;
--    sotc number(3,0);
...
--EXCEPTION
--    WHEN NO_DATA_FOUND THEN
--        RAISE_APPLICATION_ERROR(-20001, 'Khong tom thay du lieu hoc phan dang ky. ' || :new.MAHP || ' end');
--    WHEN OTHERS THEN
--        RAISE_APPLICATION_ERROR(-20001, 'Error occurred: ' || SQLERRM);
--END;
--/
...
-- SELECT * FROM DBA_ROLE_PRIVS where GRANTED_ROLE like 'RL_TRUONGHOC_GIANGVIEN';
-- SELECT * FROM DBA_ROLE_PRIVS where GRANTED_ROLE like 'RL_TRUONGHOC_TRUONGNHANSU';
-- SELECT * FROM DBA_ROLE_PRIVS where GRANTED_ROLE like 'RL_TRUONGHOC_NHANVIENCOBAN';
-- SELECT * FROM DBA_ROLE_PRIVS where GRANTED_ROLE like 'RL_TRUONGHOC_GIAOVU';
-- SELECT * FROM DBA_ROLE_PRIVS where GRANTED_ROLE like 'RL_TRUONGHOC_SINHVIEN';
