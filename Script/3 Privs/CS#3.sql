GRANT RL_TRUONGHOC_NHANVIENCOBAN TO RL_TRUONGHOC_GIAOVU;
...
CREATE OR REPLACE PROCEDURE QL_TRUONGHOC_X.DELETE_PHANCONG_GIAOVU(
    P_OLD_MAGV IN VARCHAR2, P_OLD_MAHP IN VARCHAR2, P_OLD_HK IN NUMBER, P_OLD_NAM IN NUMBER, P_OLD_CT IN CHAR 
)
AS
    V_USER CHAR(10);
    V_MAHP CHAR(10);
...
BEGIN
    ...
    FROM QL_TRUONGHOC_X.PHANCONG where MAGV = P_OLD_MAGV and
            MAHP = P_OLD_MAHP and
            HK = P_OLD_HK and
            NAM = P_OLD_NAM and
            MACT = V_MACT;
            
    IF V_MAHP IS NOT NULL AND V_MAGV IS NOT NULL THEN
        ...
        RAISE_APPLICATION_ERROR(-20001, 'Hoc Phan khong thuoc quyen quan ly cua van phong khoa');
    END IF;
...
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        RAISE_APPLICATION_ERROR(-20001, 'Khong tom thay du lieu phan cong.' || V_MAGV || '   ' || V_MAHP );
    WHEN OTHERS THEN
        RAISE_APPLICATION_ERROR(-20001, 'Error occurred: ' || SQLERRM);
END;
/
...
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        RAISE_APPLICATION_ERROR(-20001, 'Data not found: ' || 'Hoc Phan khong thuoc quyen quan ly cua van phong khoa.');
    WHEN OTHERS THEN
        RAISE_APPLICATION_ERROR(-20001, 'Error occurred: ' || SQLERRM);
END;
/
