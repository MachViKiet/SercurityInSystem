...
CREATE OR REPLACE VIEW QL_TRUONGHOC_X.DONVI_CHITIET AS
    SELECT donvi.madv, donvi.tendv, nhansu.hoten, coso.TENCS 
    FROM QL_TRUONGHOC_X.DONVI donvi
        LEFT JOIN QL_TRUONGHOC_X.TRUONGDONVI truongdv ON truongdv.MADV = donvi.MADV
        LEFT JOIN QL_TRUONGHOC_X.COSO coso ON truongdv.MACS = coso.MACS        
        LEFT JOIN QL_TRUONGHOC_X.NHANSU nhansu ON nhansu.MANV = truongdv.MANV;
...
GRANT SELECT ON THONGBAO TO RL_TRUONGHOC_NHANVIENCOBAN;
...
GRANT UPDATE ON QL_TRUONGHOC_X.DK_MY_CLASSES TO RL_TRUONGHOC_GIANGVIEN;
...
CREATE OR REPLACE PROCEDURE QL_TRUONGHOC_X.UPDATE_GRADES (
    ...
    P_HK IN NUMBER,    
    P_NAM IN NUMBER,         
    P_CT DANGKY.MACT%TYPE          
)
AS
    V_MAGV CHAR(10);
BEGIN
    V_MAGV := SYS_CONTEXT('USERENV', 'SESSION_USER');
    UPDATE QL_TRUONGHOC_X.DK_MY_CLASSES
    ...
    DBMS_OUTPUT.PUT_LINE('Cap nhat diem so thanh cong.'  || USER || ' '
    || P_MAHP || ' '
    || V_MAGV || ' '
    || P_MASV || ' '
    || P_HK || ' '
